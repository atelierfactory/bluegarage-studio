...
> **2026-09-09 以降の現在地は `HANDOFF1.md`（VESPER PIANO）を先に読むこと。** この文書は経緯と jukebox 中継（AWS）の詳細用。
...
- 2026-09-09 昼（v4.2・さとるん指摘）：**指と鍵盤のずれの根本原因は両手の親指と小指が左右逆に付いていたこと**（FINGER_Z の符号）。直したうえで、指先を鍵の上にぴったり置く計算 `_placeFinger`（3 関節を実測ヤコビアンで減衰最小二乗、関節限界で止まらないよう反対側に微分・前フレームの答えから開始・だめなら基本形からやり直し）、手の前後位置を指ごとの届く長さから決める（親指を重く）、LEAD=0.10 秒先の音で手を先回り、鍵は音の開始と同時に沈め、指はその沈んだ面に合わせる。データの指番号の間違い（同じ指に 2 音・順番が逆）はその瞬間だけ振り直し、新しい音が始まったら遠くの押さえっぱなしの音は手放す（届かない楽譜の伸ばしはペダルが保つ想定）。**`stage.rehearse(notes, pedal, beatToSec)`** が描画せずに曲を通し、音の開始と 60ms 後に指先を測る。同梱 4 曲すべて **ずれ 0**（エンターテイナー 5242 判定、エリーゼ 1810、月の光 2936、ショパン 1658。最大 x ずれ 1.6mm）。
- 同 v4.2：鍵盤の見た目（すき間 2.2mm・角の丸み 1.2mm・黒鍵は台形断面・すき間の底に黒板・鍵盤を斜めから撫でる影つきライト・押した鍵は金色に灯る）、下の帯は「既存の曲を弾く / 即興 / 作曲」の 3 つで、既存の曲を弾くを押すと「♪ 曲を選ぶ」ボタンが出る、LCD に「経過 / 全体」の時間。
- 2026-09-09 午後（v4.3）：**革命のエチュード（ショパン Op.10-12）を追加**。Mutopia の Public Domain 版（Roland Goretzki 打ち込み、Peters 1900 版が元、Mutopia-2018/05/15-743）。`.ly` も同梱して権利表記を残した。IMSLP の MIDI（CC BY 3.0）や Wikipedia の MIDI（CC BY-SA 3.0）は使わなかった。手の流れの改良：跳ぶ 60ms 前に遠くの押さえっぱなしを手放して次のかたまりに先回り、届かなかった指の残りだけで手全体をずらす（2 回まで）、片手で 10 度を超える塊は届く方の手へ（fingering.js）。同梱 5 曲リハーサル **ずれ 0**（15794 判定、最大 0.8mm）。革命の Fable 5.1 演奏解釈は `window.vesperInterpret` で実行中/済み（下を参照）。
