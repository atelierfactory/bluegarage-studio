...
`public/piano/` is a **piano-only** tool built from the same parts. The left two thirds are a single-track piano roll plus a theme bar (improvise / compose, with a choice of model and depth); the right side is a 3D stage where the robot pianist VESPER-01 (three.js) plays a grand piano (top: an orbitable wide shot, bottom: the fingertips; full-screen splits them left/right).
...
**Existing pieces**: public-domain classics (The Entertainer, Für Elise, Clair de Lune, Chopin's Trois Nouvelles Études No. 1 — Mutopia Project public-domain editions). The Entertainer ships as a **performance interpretation by Claude Fable 5.1**: the score is untouched, and the model decided every dynamic, micro-timing, articulation, hand, finger and pedal (`repertoire/entertainer.vesper.json`; reproducible in the browser with `window.vesperInterpret({model})`). Your own MIDI files load too (hands/fingers assigned automatically, CC64 pedal read). Copyrighted works are neither bundled nor generated.
**Improvisation banks**: `public/piano/banks/` holds 20 styles / 1,320 patterns (progressions, left-hand patterns, right-hand motifs, fills, written in scale degrees; generated by Fable 5.1 via `tools/make-banks.mjs`). The closest style to your theme is picked and mixed locally for hours.
