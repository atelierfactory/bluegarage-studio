...
const view=process.argv[2]??'knob',beat=Number(process.argv[3]??song.shots[view]??song.shots.knob),w=960,h=640;stage.w=w;stage.h=h;stage.poseAt(beat,b=>beatToSec(b,song));stage.setShot(view);stage.scene.updateMatrixWorld(true);const camera=stage.shotCamera;camera.updateMatrixWorld();
...
stage.scene.traverseVisible(mesh=>{if(!mesh.isMesh)return;if(mesh.isInstancedMesh){for(let i=0;i<mesh.count;i++){mesh.getMatrixAt(i,inst);world.multiplyMatrices(mesh.matrixWorld,inst);raster(mesh,world);}}else raster(mesh,mesh.matrixWorld);});
const dir=new URL('../astra/shots/',import.meta.url);fs.mkdirSync(dir,{recursive:true});const png=new URL(`geometry-${view}${process.argv[3]?`-${beat}`:''}.png`,dir),ppm=Buffer.concat([Buffer.from(`P6\n${w} ${h}\n255\n`),image]);execFileSync('/opt/homebrew/bin/ffmpeg',['-hide_banner','-loglevel','error','-y','-f','image2pipe','-vcodec','ppm','-i','pipe:0',png.pathname],{input:ppm});console.log(`${png.pathname} (geometry diagnostic only)`);
