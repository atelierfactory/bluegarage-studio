...
import { buildBlueprintRequest, buildPianoRequest } from "../js/prompts.js";
...
const statusLines = [];
function status(msg, cls = "") { statusLines.push({ msg, cls }); while (statusLines.length > 3) statusLines.shift(); $("#status-text").innerHTML = statusLines.map((l) => `<span class="${l.cls}">${esc(l.msg)}</span>`).join("\n"); }
function progress(msg) { $("#status-text").innerHTML = statusLines.slice(-2).map((l) => `<span class="${l.cls}">${esc(l.msg)}</span>`).join("\n") + (statusLines.length ? "\n" : "") + `<span class="lit">${esc(msg)}</span>`; }
...
function loadMidiIntoSong(buffer, title, meta = {}) {
  ...
  const a = checkNow();
  return `「${title}」(${notes.length}音、${bars}小節、${song.tempo} BPM) 手と指は自動。検査: ${describeAnalysis(a)}`;
}
...
async function playRepertoire(item) {
  ...
  return loadMidiIntoSong(await r.arrayBuffer(), `${item.title} — ${item.composer}`, { credit: `${item.license} · ${item.source}` });
}
...
async function jamStart(theme, { model } = {}) {
  ...
  await lib.saveSong(ensureSongId(), state.song).catch(() => {});
  state.songId = lib.newId();
  const key = state.song.key || "C major", tempo = state.song.tempo || 96;
  const song = pianoSong();
  song.title = theme ? `即興 — ${theme.slice(0, 40)}` : "即興"; song.key = key; song.tempo = tempo;
  song.sections = [{ name: "JAM", bars: 8, description: "" }];
  resetSong(song); const tr = pianoTrack(); emit("selection");
  const cached = theme ? loadBanks()[theme] : null;
  const imp = new Improviser(cached ?? DEFAULT_BANK, { density: 1 });
  jam = { imp, timer: null, ahead: 0, theme };
  for (let i = 0; i < 2; i++) jamAppend(tr, imp.next8(jam.ahead * 4));
  await play(0);
  jam.timer = setInterval(jamTick, 500);
  status(theme ? (cached ? `即興を始めました (お題「${theme}」の手札)` : `即興を始めました。まず既定の手札で弾き、お題「${theme}」の手札ができたら切り替えます`) : "即興を始めました (既定の手札)", "lit");
  if (theme && !cached) {
    makeBank(theme, key, tempo, model).then((bank) => { if (jam && jam.theme === theme) { jam.imp.bank = bank; jam.imp.lh = bank.lhPatterns[0]; jam.imp.rh = null; jam.imp.rhLeft = 0; jam.imp.progIdx = 0; status(`お題「${theme}」の手札に切り替えました (進行 ${bank.progressions.length} / 伴奏 ${bank.lhPatterns.length} / モチーフ ${bank.rhMotifs.length})`, "lit"); } })
      .catch((e) => status(`手札の生成に失敗 (${e.message})。既定の手札で続けます`));
  }
}
...
/* ─────────── お題バー ─────────── */
let mode = "jam";
document.querySelectorAll(".mtab").forEach((b) => b.addEventListener("click", () => { mode = b.dataset.mode; document.querySelectorAll(".mtab").forEach((x) => x.classList.toggle("on", x === b)); $("#sel-effort").style.display = mode === "compose" ? "" : "none"; $("#btn-go").textContent = mode === "compose" ? "♪ 作曲する" : "▶ 即興を始める"; }));
$("#sel-model").innerHTML = MODELS.map((m) => `<option value="${m.id}">${m.label}</option>`).join("");
$("#sel-model").value = settings.get().model ?? MODELS[0].id;
$("#sel-effort").style.display = "none"; $("#btn-go").textContent = "▶ 即興を始める";
let running = false;
...
async function boot() {
  ...
  const tr = await resolveTransport();
  status(`VESPER PIANO (${tr === "direct" ? "自分の API キー" : tr === "proxy" ? "サーバーのキー" : "キー未設定 — ⚙ から入れてください"})`);
