...
const DICT = {
  ja: {
    ...
    "set.remote": "このサイトには専用の中継サーバーが用意されているので、キー無しで使えます (自分のキーを入れると自分のキーが優先されます)。",
    "set.remote.down": "このサイトの中継サーバーに今つながりません。自分の API キーを入れると使えます。",
    "set.remote.songs": "今日はあと {left} 曲作れます (1日 {limit} 曲まで。全員合計)。",
    "set.pass": "アクセスコード", "set.pass.help": "このサイトの管理者から教えてもらったコード。自分のキーを使う場合は不要です。",
  ...
  en: {
    ...
    "set.remote": "This site has its own relay server, so it works without a key (your own key takes priority if you enter one).",
    "set.remote.down": "The site's relay server is unreachable right now. Enter your own API key to continue.",
    "set.remote.songs": "{left} songs left today ({limit} per day, shared by all visitors).",
    "set.pass": "Access code", "set.pass.help": "The code you got from the site owner. Not needed if you use your own key.",
