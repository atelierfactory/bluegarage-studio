...
export const KNOB_IDS = ["cutoff", "resonance", "envAmount", "lfoRate", "lfoDepth", "drive", "delay", "reverb"];
export const LFO_DIVISIONS = [["4/1", 16], ["2/1", 8], ["1/1", 4], ["1/2", 2], ["1/4d", 1.5], ["1/4", 1], ["1/8d", 0.75], ["1/8", 0.5], ["1/8t", 1 / 3], ["1/16", 0.25]];
const clamp01 = (v) => (Number.isFinite(+v) ? Math.max(0, Math.min(1, +v)) : 0.5);
export function knobToCutoff(v) { return 20 * Math.pow(900, clamp01(v)); }            // 20 Hz .. 18 kHz (log)
export function cutoffToKnob(hz) { return clamp01(Math.log(Math.max(20, hz) / 20) / Math.log(900)); }
export function knobToLfoRate(v) { return 0.05 * Math.pow(400, clamp01(v)); }         // 0.05 .. 20 Hz (log)
export function lfoRateToKnob(hz) { return clamp01(Math.log(Math.max(0.05, hz) / 0.05) / Math.log(400)); }
export function knobToDivision(v) { return LFO_DIVISIONS[Math.round(clamp01(v) * (LFO_DIVISIONS.length - 1))][0]; }
export function divisionToKnob(div) { const i = LFO_DIVISIONS.findIndex((d) => d[0] === div); return (i < 0 ? 5 : i) / (LFO_DIVISIONS.length - 1); }
export function divisionBeats(div) { const d = LFO_DIVISIONS.find((x) => x[0] === div); return d ? d[1] : 1; }
export const ENV_OCTAVES = 6; // envAmount knob 1.0 = 6 octaves
...
export const WAVETABLE_NAMES = Object.keys(WT_DEFS);
...
export function knobDefaultsFromPatch(p) {
...
export const DEFAULT_CORE_PATCH = {
