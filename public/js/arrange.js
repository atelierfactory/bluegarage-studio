...
export class Arrange {
  ...
  draw() {
    ...
    // ── セクション帯
    let bar = 0;
    const secColors = ["rgba(47,123,255,.16)", "rgba(56,195,255,.10)", "rgba(124,92,255,.14)", "rgba(67,230,160,.10)", "rgba(255,200,87,.10)"];
    (song.sections ?? []).forEach((s, i) => {
      const x = bar * ppb, sw = s.bars * ppb;
      ctx.fillStyle = secColors[i % secColors.length];
      ctx.fillRect(x, 0, sw, h);
      ctx.strokeStyle = "rgba(47,123,255,.4)";
    ...
    if (tracks.length) {
      ...
      tracks.forEach((t, i) => {
        ...
        if (sel) {
          ctx.fillStyle = "rgba(47,123,255,.10)";
      ...
      ctx.fillStyle = "#3c5580"; ctx.font = "12px 'Zen Kaku Gothic New'";
    ...
    ctx.shadowColor = "#38c3ff"; ctx.shadowBlur = 5;
